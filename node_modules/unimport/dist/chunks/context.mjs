...
function createUnimport(opts) {
  ...
  function generateTypeDeclarations(options) {
    const opts2 = {
      resolvePath: (i) => i.from.replace(/\.ts$/, ""),
      ...options
    };
    let dts = toTypeDeclarationFile(ctx.imports, opts2);
    for (const addon of ctx.addons) {
      dts = addon.declaration?.call(ctx, dts, opts2) ?? dts;
    }
    return dts;
  }
  updateImports();
...
async function detectImports(code, ctx) {
  ...
  const identifiers = new Set(Array.from(strippedCode.matchAll(matchRE)).map((i) => i[1] === "." ? "" : i[2]).filter(Boolean));
