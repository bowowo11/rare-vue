...
function createUnimport(opts) {
  ...
  function generateTypeDeclarations(options) {
    ...
    let dts = vueTemplate.toTypeDeclarationFile(ctx.imports, opts2);
...
async function detectImports(code, ctx) {
  ...
  const identifiers = new Set(Array.from(strippedCode.matchAll(vueTemplate.matchRE)).map((i) => i[1] === "." ? "" : i[2]).filter(Boolean));
